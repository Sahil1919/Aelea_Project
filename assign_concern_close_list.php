...
<!--                               <th>Edit</th>-->
      ...
      if ($_SESSION['User_type'] == 'management' || $_SESSION['User_type'] == 'admin'){
        $qry = mysqli_query($connection, "SELECT * FROM assign_concern where status='Close' order by work_assign_date desc") or die("select query fail" . mysqli_error($connection));
$count = 0;
...
$sess_report_id = $_SESSION['user'];
        $qry = mysqli_query($connection, "SELECT DISTINCT * FROM assign_concern where assign_concern.emp_id='$sess_report_id' and assign_concern.status='Close'  ")
...
$qry = mysqli_query($connection, "SELECT DISTINCT * FROM assign_concern where assign_concern.userid='$sess_report_id' and assign_concern.status='Close' ") or die("select query fail" . mysqli_error($connection));
// $count = 0;
date_default_timezone_set('Asia/Kolkata');
$date = date('d-m-y g:i:s A');
while ($row = mysqli_fetch_assoc($qry)) {
