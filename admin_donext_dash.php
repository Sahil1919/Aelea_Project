...
if ($_SESSION['User_type'] =='management' || $_SESSION['User_type'] =='admin'){
    ...
    $qry = "SELECT id,task_id FROM emp_login,assign_task where user_role IN ('employee','reporting manager') and assign_task.status='Open' and emp_id=id and report_to='$sess_report_id'";
    // $open_task = 0;
    if ($result = mysqli_query($connection, $qry)) {
        $Total_emp_task_open = mysqli_num_rows($result);
    }

    $retailer_account = "SELECT DISTINCT task_id FROM emp_login,assign_task where user_role IN ('reporting manager') and assign_task.status='Open' and emp_id='$sess_report_id'";
    ...
    $qry = "SELECT id,task_id FROM emp_login,assign_task where user_role IN ('employee','reporting manager') and assign_task.status='WIP' and emp_id=id and report_to='$sess_report_id'";
    // $open_task = 0;
    if ($result = mysqli_query($connection, $qry)) {
        $Total_emp_task_wip = mysqli_num_rows($result);
    }

    $retailer_account = "SELECT DISTINCT task_id FROM emp_login,assign_task where user_role IN ('reporting manager') and assign_task.status= 'WIP' and emp_id='$sess_report_id'";
    $Total_rm_task_wip = 0;
    if ($result = mysqli_query($connection, $retailer_account)) {
    ...
    $qry = "SELECT id,task_id FROM emp_login,assign_task where user_role IN ('employee','reporting manager') and assign_task.status='Cancel' and emp_id=id and report_to='$sess_report_id'";
    // $open_task = 0;
    if ($result = mysqli_query($connection, $qry)) {
        $Total_emp_task_cancel = mysqli_num_rows($result);
    }

    $retailer_account = "SELECT DISTINCT task_id FROM emp_login,assign_task where user_role IN ('reporting manager') and assign_task.status= 'Cancel' and emp_id='$sess_report_id'";
