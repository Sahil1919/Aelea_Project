...
$pdf->SetHeaderData(PDF_HEADER_LOGO, PDF_HEADER_LOGO_WIDTH, PDF_HEADER_TITLE.' 001', PDF_HEADER_STRING, array(0,64,255), array(0,64,18));
...
{
	// EMP Code Logic
    $qry = mysqli_query($connection, "SELECT emp_code FROM concern_views ") or die("select query fail" . mysqli_error());
    ...
    while($row= mysqli_fetch_assoc($qry))
	{
		...
		{
            $data = $row['emp_code'];
            $qry1 = mysqli_query($connection, "SELECT * FROM concern_views WHERE emp_code='$data' ") or die("select query fail" . mysqli_error());
            $count = 0;
			$result=mysqli_query($connection,"SELECT count(*) as total from concern_views WHERE emp_code='$data'");
	...
	// EMP Name Logic
    $qry = mysqli_query($connection, "SELECT emp_name FROM concern_views ") or die("select query fail" . mysqli_error());
    while($row= mysqli_fetch_assoc($qry)){
        // $name = strtolower($row['emp_name']);
        if (str_starts_with(strtolower($row['emp_name']),strtolower($params['search'])))
		{
            $data = $row['emp_name'];
            $qry1 = mysqli_query($connection, "SELECT * FROM concern_views WHERE emp_name='$data' ") or die("select query fail" . mysqli_error());
            $count = 0;
			$result=mysqli_query($connection,"SELECT count(*) as total from concern_views WHERE emp_name='$data' ");
			$data=mysqli_fetch_assoc($result);
			$row_count = $data['total'];
			while($row= mysqli_fetch_assoc($qry1))
			{
    ...
    $qry = mysqli_query($connection, "SELECT task FROM concern_views ") or die("select query fail" . mysqli_error());
    while($row= mysqli_fetch_assoc($qry)){
        ...
        if (str_starts_with($task,strtolower($params['search'])))
		{
            ...
            $data = $row['task'];
            $qry1 = mysqli_query($connection, "SELECT * FROM concern_views WHERE task='$data' ") or die("select query fail" . mysqli_error());
            $count = 0;
			$result=mysqli_query($connection,"SELECT count(*) as total from concern_views WHERE task='$data' ");
			$data=mysqli_fetch_assoc($result);
			$row_count = $data['total'];
			while($row= mysqli_fetch_assoc($qry1))
			{
...
else
{
    ...
    $qry = mysqli_query($connection, "SELECT * FROM concern_views ") or die("select query fail" . mysqli_error());
	$count = 0;
	$result=mysqli_query($connection,"SELECT count(*) as total from concern_views");
	$data=mysqli_fetch_assoc($result);
	$row_count = $data['total'];
    while ($row = mysqli_fetch_assoc($qry)) 
    {   
