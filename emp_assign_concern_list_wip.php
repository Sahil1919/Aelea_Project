...
<div class="content-i">
  ...
  <table id="example" style="width: 100%;" class="display table table-bordered table-responsive" style="width:100%">
        <thead>
                 ...
                 $qry = mysqli_query($connection, "SELECT * FROM assign_concern where status='WIP' and assignby='$assignby'  order by work_assign_date desc") or die("select query fail" . mysqli_error($connection));
$count = 0;
...
<?php }?>
                 ...
                 $qry = mysqli_query($connection, "SELECT * FROM assign_concern where status='WIP' and emp_id='$emp_id' order by work_assign_date desc") or die("select query fail" . mysqli_error($connection));
while ($row = mysqli_fetch_assoc($qry)) {
    $count = $count + 1;
    $task_id = $row['task_id'];
            $emp_id1 = $row['emp_id']; 
